...
<link rel="prefetch" href="{{$item['src']}}" />
@endforeach
@endpush
...
<div x-data="picker({{json_encode($items)}})" class="text-center">
    <template x-if="started">
        <div>
            <img class="mx-auto border-8 border-yellow-300 h-80" :src="`${item.src}`" />
            <p x-show="stopped" x-text="item.name" class="mt-4 text-white font-display"></p>
        </div>
    </template>
    <template x-if="!started">
